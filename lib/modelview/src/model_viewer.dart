...
class _ModelViewerState extends State<ModelViewer> {
  ...
  Future<void> _initProxy() async {
    ...
    final chain = await _readAsset("etc/ssl/server.pem");
    final privateKey = await _readAsset("etc/ssl/private.pem");
    securityContext.useCertificateChainBytes(chain.cast<int>());
    securityContext.usePrivateKeyBytes(privateKey.cast<int>());

    _proxy = await HttpServer.bindSecure(
        InternetAddress.loopbackIPv4, 11443, securityContext,
        shared: true);
